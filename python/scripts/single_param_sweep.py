...
def main():
    ...
    sim_result_path = os.path.join(
        curr_path, "../../liberate/lib/example_tt_1.0_70_nldm.lib"
    )
    sim_file = File(sim_result_path)

    reference_netlist_name = "INVX1"
    reference_netlist_path = os.path.join(
        curr_path, f"../../liberate/netlist_ref/{reference_netlist_name}.sp"
    )
    ...
    def persist_netlist_in_run_dir(netlist: Netlist):
        path = os.path.join(
            curr_path, f"../../liberate/netlist_wrk/{netlist.cell_name}.sp"
        )
    ...
    single_param_sweep = SingleParamSweep(
        cost_function, candidate_generator, liberty_parser, sim_file
    )
